...
export default function NotionImage({ block, nested }: Props) {
  ...
  if (caption === 'wide') {
    className += `h-[300px] -mx-[32px] w-[calc(100%+64px)] md:h-[400px] md:-mx-[58px] md:w-[calc(100%+116px)] mt-12 ${!nested && 'mb-12'}`
  } else if (caption === 'tight') {
    className += `h-[280px] mt-12 ${!nested && 'mb-12'} w-full`
  } else if (caption === 'high') {
    className += `h-[500px] md:h-[800px] mt-12 ${!nested && 'mb-12'} w-full`
  } else {
    className += `h-[450px] mt-12 ${!nested && 'mb-12'} w-full`
  }
  ...
  return (
    <div className={`${className} relative`}>
      ...
      <Image src={getImageURL(block.image)} alt={'Notion Image'} fill />
    </div>
  )
}
