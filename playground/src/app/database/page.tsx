...
export default async function Database() {
  ...
  const { database, entries } = await query.retrieveDatabase(
    process.env['NOTION-PLAYGROUND-DATABASE']!
  )
      ...
      <div className="grid grid-cols-2 gap-3 mt-12">
        {pages.map((page) => (
          <article key={page.id} className="p-4 border border-gray-200 rounded mb-4">
            ...
            <Link
              href={'/database/' + NotionParser.getPlainTextPageProperty(page.properties, 'Slug')}
              className="w-full"
            >
